...
/*
 java.lang.String
 replace("OLD","NEW) 는 OLD 글자를 NEW 글자로 바꾸는 역할을 한다.
 */
public class ReplaceDemo {
	public static void main(String[] args) {
		System.out.println("Hello Java".replace("Java", "JSP"));
		/*
		 JAVA 버전 5 이전에는 replace() 는 알파벳 한 글자만 바꾸는 기능만 있었으나, replaceAll() 이 등장.
		 replaceAll() 은 여러 글자수를 바꾸는데 사용되었으나, 이후 replace()도 
		 여러 글자수를 바꾸는 기능으로 확대되어 현재는 별 차이가 없다.
		 다만, replaceAll() 은 정규표현식까지도 바꾸는 기능이 있어서 
		 되도록이면 replace() 보다는 replaceAll() 을 쓰는것을 권장한다.
		 */
